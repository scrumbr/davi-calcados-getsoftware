...
import br.com.getsoftware.davicalcados.gui.edit.EditSaidasGUI;
 ...
 * @author Andrey Gois
 */
public class ListSaidasGUI extends javax.swing.JFrame {

    private ArrayList<Saida> listSaidas;
    private ArrayList<Saida> listSaidasDate;
    private int linhaSelecionada = 0;
    private TelaMenuGUI telaMenu;

    public ListSaidasGUI() throws SQLException {
        initComponents();
        jTable1.setSelectionBackground(Color.red); //fundo da linha
        dadosTabela();
    }

    public ListSaidasGUI(TelaMenuGUI telaMenu) throws SQLException {
        this();
    ...
    private void jBEditarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBEditarActionPerformed
        ...
        try {
            saida = SaidaBO.getById(Integer.valueOf(jTable1.getValueAt(linhaSelecionada, 0).toString()));
           this.setEnabled(false);
            new EditSaidasGUI(this, saida).setVisible(true);
    ...
    private void jBExcluirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBExcluirActionPerformed
        ...
        if (opc == JOptionPane.YES_OPTION) {
            try {
                SaidaBO.delete(Integer.valueOf(jTable1.getValueAt(linhaSelecionada, 0).toString()));
                dadosTabela();
                atualizaLinhaSelecionada();
                tabelaVazia();
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        if(jFDataDe.getText().equals("  /  /    ") || jFDataAte.getText().equals("  /  /    ")){
             consulta = "select  s.id_saida, s.descricao_saida, s.valor_saida, s.data_saida, u.username from saida as s, usuario as u where s.id_usuario = u.id_usuario;";
        }else{
             consulta = "select  s.id_saida, s.descricao_saida, s.valor_saida, s.data_saida, u.username from saida as s, "
                    + "usuario as u where s.id_usuario = u.id_usuario and s.data_saida BETWEEN " + "'"+ (TransformDate.transformDate(jFDataDe.getText())) +"'" + "AND" + "'" + (TransformDate.transformDate(jFDataAte.getText()))+ "'";
        }
         try {
                GenericReport saidaRel = new GenericReport(consulta, "SaidaReport.jasper");
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(ListSaidasGUI.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                try {
                    new ListSaidasGUI().setVisible(true);
                } catch (SQLException ex) {
                    Logger.getLogger(ListSaidasGUI.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
        });
    }
    ...
    public void dadosTabela() throws SQLException {
        ...
        for (int i = 0; i < listSaidas.size(); i++) {
            modelo.addRow(new Object[]{
                listSaidas.get(i).getIdSaida(),
                listSaidas.get(i).getDescricao(),
                listSaidas.get(i).getIdUsuario(),
                listSaidas.get(i).getValorSaida(),
                TransformDate.transformDate(listSaidas.get(i).getDataSaida()),});
            //listUsuarios.get(i).getNivel() == 1 ? "ADMINISTRADOR" : "FUNCIONARIO" ,});
        }
    }

    public void dadosTabelaDate() throws SQLException {
        listSaidasDate = SaidaBO.listAllDate(TransformDate.transformDate(jFDataDe.getText()), TransformDate.transformDate(jFDataAte.getText()));
        tabelaVazia();

        DefaultTableModel modelo = (DefaultTableModel) jTable1.getModel();
        modelo.setNumRows(0);

        for (int i = 0; i < listSaidasDate.size(); i++) {
            modelo.addRow(new Object[]{
                listSaidasDate.get(i).getIdSaida(),
                listSaidasDate.get(i).getDescricao(),
                listSaidasDate.get(i).getIdUsuario(),
                listSaidasDate.get(i).getValorSaida(),
                TransformDate.transformDate(listSaidasDate.get(i).getDataSaida()),});
            //listUsuarios.get(i).getNivel() == 1 ? "ADMINISTRADOR" : "FUNCIONARIO" ,});
        }
    }
}
